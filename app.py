...
def handle_api_error(error_message, operation_name="operation"):
    """Handle TwelveLabs API errors with user-friendly messages"""
    if "429" in error_message or "rate limit" in error_message.lower():
        st.error(f"🚫 **Rate Limit Exceeded - {operation_name}**")
        st.warning("""
        **You've reached your TwelveLabs API daily limit (50 requests/day).**
        
        **Options:**
        - ⏰ **Wait**: Rate limit resets at 18:18 UTC today
        - 🔄 **Upgrade**: Check your TwelveLabs dashboard for higher plans  
        - 💡 **Optimize**: Use existing videos instead of uploading new ones
        """)
        with st.expander("🔍 Rate Limit Details"):
            st.code(error_message)
    else:
        st.error(f"Error in {operation_name}: {error_message}")
...
st.markdown("<h2 style='text-align: center; font-size: 40px; color: white;'><strong>HootHive: Chat with Videos ✍🤖</strong></h2>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; font-size: 20px; color: #99ccff;'>AI-powered learning assistant that transforms recorded lectures into structured, accessible, and collaborative study resources</p>", unsafe_allow_html=True)
...
def process_qa_search():
    ...
    if query and st.button("Search Video(s)", key="search_qa_button", disabled=search_disabled):
        try:
            ...
            handle_api_error(str(e), "video search")
    ...
    with col1:
        if st.button("📝 Generate Summary", key="gen_summary_btn"):
            try:
                ...
                handle_api_error(str(e), "summary generation")
    ...
    with col2:
        if st.button("📑 Generate Chapters", key="gen_chapters_btn"):
            try:
                ...
                handle_api_error(str(e), "chapters generation")
    ...
    with col3:
        if st.button("✨ Generate Highlights", key="gen_highlights_btn"):
            try:
                ...
                handle_api_error(str(e), "highlights generation")
    ...
    with col1:
        if st.button("🔍 Analyze", key="custom_analysis_btn", disabled=not custom_prompt):
            try:
                ...
                handle_api_error(str(e), "custom analysis")
